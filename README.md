...
In the `Odds` block, `runner-scan` should show `TA` rather than a generated `Codex` line. When both `TA vs Oddset` and `Vitel vs Oddset` are present, `TA vs Oddset` is the main public edge line and `Spelidé` should follow that line first, while `Vitel vs Oddset` remains a secondary experimental comparison.
