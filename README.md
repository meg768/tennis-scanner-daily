...
In the `Odds` block, `runner-scan` should show `Tennis Abstract` rather than a generated `Codex` line. When both `Tennis Abstract/Oddset` and `Vitel/Oddset` are present, `Tennis Abstract/Oddset` is the main public edge line and `Spelidé` should follow that line first, while `Vitel/Oddset` remains a secondary experimental comparison.
